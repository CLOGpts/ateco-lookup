...
@pytest.fixture
def app(mock_risk_service):
    """Create a test FastAPI app with risk router and mocked service"""
    app = FastAPI()
    app.include_router(risk_router.router)
    # Override dependency to use mock
    app.dependency_overrides[risk_router.get_risk_service] = lambda: mock_risk_service
    return app
...
def test_save_assessment_success(client, mock_risk_service):
    ...
    mock_risk_service.calculate_risk_score.return_value = {
        ...
        "final_score": 33,
        "analysis": "Livello di rischio: Medium (Score: 33/100). Monitorare e valutare opzioni"
    }
...
def test_save_assessment_high_risk(client, mock_risk_service):
    ...
    mock_risk_service.calculate_risk_score.return_value = {
        ...
        "final_score": 105,
        "analysis": "Livello di rischio: Critical (Score: 105/100). Richiede azione immediata"
    }
...
def test_calculate_assessment_success(client, mock_risk_service):
    ...
    mock_risk_service.calculate_risk_matrix.return_value = {
        ...
        "risk_color": "orange",
        "risk_value": 0,
        "inherent_risk": {"value": 2, "label": "High"},
        "control_effectiveness": {"value": 3, "label": "+", "description": "Sostanzialmente adeguato"},
        "calculation_details": {},
        "recommendations": ["Priorità alta per mitigazione", "Rafforzare i controlli esistenti"]
    }
    ...
    data = response.json()
    assert data["status"] == "success"
    assert data["matrix_position"] == "C3"
